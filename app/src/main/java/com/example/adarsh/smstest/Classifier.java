...
public abstract class Classifier<T, K> implements IFeatureProbability<T, K> {
    ...
    public void incrementFeature(T feature, K category) {
        ...
        if (features == null) {
            ...
            features = this.featureCountPerCategory.get(category); //returns a list of words under that caterogy
        }
        ...
        features.put(feature, ++count); // incrementing the frequency of that word under that category
        ...
        this.totalFeatureCount.put(feature, ++totalCount); // incrementing the overall frequency of that word
    }
    ...
    public void learn(Classification<T, K> classification) {
        ...
        if (this.memoryQueue.size() > this.memoryCapacity) {
            ...
            this.decrementCategory(toForget.getCategory()); // to remove the words onver memory overflows
        }
    }
